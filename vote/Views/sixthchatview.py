...
# def siroom(request, siroom):
#     username = request.GET.get('username')
#     # room_details = Room.objects.get(name=room)
#     return render(request, 'chat/siroom.html', {
#         'username': username,
#         # 'room': room,
#         # 'room_details': room_details
#     })

# def sicheckview(request):
...
#         for i in key7:
#             z=i.group_id
#     new_message = sixthMessage.objects.create(value=message, user=request.user.name,room=z)
#     new_message.save()
#     return HttpResponse('Message sent successfully')


# def sigetMessages(request):
#     # room_details = Room.objects.get(name=room)
#     key1=pod_groups_members.objects.filter(member_id=request.user.id) 
...
#     messages = sixthMessage.objects.filter(room=z)
